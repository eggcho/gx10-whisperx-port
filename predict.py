...
def resolve_runtime():
    # Keep original CUDA-first behavior when CUDA-enabled CTranslate2 is present.
    try:
        ctranslate2.get_supported_compute_types("cuda")
        return "cuda", "float16"
    except Exception as exc:
        print(f"CUDA backend unavailable in CTranslate2, falling back to CPU: {exc}")
        return "cpu", "int8"
...
if torch_device != "cuda":
    print(f"PyTorch CUDA backend unavailable; VAD/alignment/diarization will run on {torch_device}.")
...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        with torch.inference_mode():
            ...
            vad_runtime_model = whisperx_vad.load_vad_model(torch.device(torch_device), **vad_options)
            model = whisperx.load_model(whisper_arch, device, compute_type=compute_type, language=language,
                                        asr_options=asr_options, vad_options=vad_options, vad_model=vad_runtime_model)
            ...
            if debug and torch_cuda_available():
                print(f"max gpu memory allocated over runtime: {torch_max_memory_gb():.2f} GB")
...
def detect_language(full_audio_file_path, segments_starts, language_detection_min_prob,
    ...
    model = whisperx.load_model(whisper_arch, device, compute_type=compute_type, asr_options=asr_options,
                                vad_options=vad_options, vad_model=vad_runtime_model)
...
def align(audio, result, debug):
    ...
    model_a, metadata = whisperx.load_align_model(language_code=result["language"], device=torch_device)
...
def diarize(audio, result, debug, huggingface_access_token, min_speakers, max_speakers):
    ...
    diarize_model = whisperx.DiarizationPipeline(use_auth_token=huggingface_access_token, device=torch_device)
